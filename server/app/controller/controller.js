...
exports.update_a_board = function (req, res) {
    ...
    Boards.updateBoard(new Boards(newBoard), function (err, result) {
        console.log(result);
...
exports.delete_a_board = function (req, res) {
    const new_board = new Boards(req.body)
    Boards.deleteBoard(new_board, function (err, result) {
        res.send({result:result})
    });
};
//##############################################################################################################
...
exports.insert_comment = function (req, res) {
    const { newComment, board_no } = req.body.data
    Comments.insertComment(newComment,board_no, function (err, result) {
        err ? res.send(err) : res.send(result)
    })
