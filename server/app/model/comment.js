...
Comments.insertComment = function (newComment,board_no, result) {
    //const { board_no, content } = newComment
    //, id_, pw, dept_name, name_, rank_
    const params = [board_no, newComment ]
    const sql = "insert into rdb.comment2 ( board_no, content, reg_date) values (?, ?,now())"
    mysql.query(sql,params, function (err, queryResult) {
        console.log(queryResult);
            err? result(err, null) : result(null,queryResult)//insert -> result row가 없음
        })
}
