...
/*
    totalCount : 전체 게시글 갯수(sql count로 얻기)
    pageSize : 1 page당 보여줄 게시글 갯수
    page : 호출하고자 하는 페이지(1 이상)
    totalPage : 페이지 총 겟수
    totalPage = (totalCount / pageSize) + (totalCount % pageSize == 0 ? 0:1)
    startCount(불러오고자 하는 게시글 시작 카운트) = (pagtotalCount : 전체 게시글 갯수(sql count로 얻기)
    pageSize : 1 page당 보여줄 게시글 갯수
    page : 호출하고자 하는 페이지(1 이상)
    totalPage : 페이지 총 겟수
    totalPage = (totalCount / pageSize) + (totalCount % pageSize == 0 ? 0:1)
    startCount(불러오고자 하는 게시글 시작 카운트) = (page - 1) * pageSize + 1
    endCount(불러오고자 하는 게시글 끝 카운트) = page * pageSizee - 1) * pageSize + 1
    endCount(불러오고자 하는 게시글 끝 카운트) = page * pageSize
